define(["jquery","language_module", "mapwrapper_module", "rest_module","navigation_module"], function(jeq, lang, mapwrap, rest, navmod)
{
    return function ViewMarketMap()
    {
        var map;
        /**
        \brief initializes the ViewMarketMap module
        \
        */
        this.init = function()
        {
            ...
            navigator.geolocation.getCurrentPosition(onCurrentPositionSuccess, function(error){console.log("failed to get position - error: "+error.message);});

        };
        /**
            \brief Populates the map with all of our markets.
            \pre Map should be initialized.
                Markets should be a list of markets with every market as a javascript object.
        */
        populateMarketMap = function(markets)
        {
            for (var i = 0; i < markets.length; i++)
            {
                ...
                /** 
            \brief Adds the users current position to the map and zooms into the location.
            \pre Map should be initialized.
        */
