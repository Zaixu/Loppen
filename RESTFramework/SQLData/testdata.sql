...
-- Data
-- :Users
INSERT INTO users (name, email, password_hash, salt, created_on, modified_on)
SELECT * FROM (SELECT 'Crynac_Bot', 'crynacbot@crynac.com', 'sadasfsa', 'asd', CURRENT_TIMESTAMP(), NOW()) AS tmp
WHERE NOT EXISTS (
    SELECT name FROM users WHERE name = 'Crynac_Bot'
) LIMIT 1;

-- :Items
INSERT INTO fleamarket_items(name)
SELECT * FROM(SELECT 'furniture') AS tmp
WHERE NOT EXISTS ( SELECT name FROM fleamarket_items WHERE name = 'furniture' ) LIMIT 1;

INSERT INTO fleamarket_items(name)
SELECT * FROM(SELECT 'antic') AS tmp
WHERE NOT EXISTS ( SELECT name FROM fleamarket_items WHERE name = 'antic' ) LIMIT 1;

INSERT INTO fleamarket_items(name)
SELECT * FROM(SELECT 'clothes') AS tmp
WHERE NOT EXISTS ( SELECT name FROM fleamarket_items WHERE name = 'clothes' ) LIMIT 1;

INSERT INTO fleamarket_items(name)
SELECT * FROM(SELECT 'other') AS tmp
WHERE NOT EXISTS ( SELECT name FROM fleamarket_items WHERE name = 'other' ) LIMIT 1;
